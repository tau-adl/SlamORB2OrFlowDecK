...
float xLastupdate; //dann
float yLastupdate; //dann
int FirstRoundSlam; //dann
int SlamOrFlow;//dan
...
static void extPositionHandler(CRTPPacket* pk)
{
  if (!SlamOrFlow)//dann
  {
	  static float xO,yO; //dann
	  const struct CrtpExtPosition* data = (const struct CrtpExtPosition*)pk->data;
	  if( FirstRoundSlam == 12 && isTrue)
	  {

		  xOffset=(xLastupdate-data->x);//dann
		  xO=xOffset;
		  yOffset=(yLastupdate-data->y);//dann
		  yO=yOffset;
		  isTrue=false;

	  }
	  ext_pos.x = data->x+xO;//dann
	  ext_pos.y = data->y+yO;//dann
	  ext_pos.z = data->z;
	  ext_pos.stdDev = extPosStdDev; //dann

...
void estimatorKalman(state_t *state, sensorData_t *sensors, control_t *control, const uint32_t tick)
{
  ...
  while (stateEstimatorHasFlowPacket(&flow) && SlamOrFlow) //dann
  {
    kalmanCoreUpdateWithFlow(&coreData, &flow, sensors); // dann
    xLastupdate=state->position.x;//dann
    yLastupdate=state->position.y;//dann
    doneUpdate = true;
  }
